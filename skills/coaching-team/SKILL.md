...
propriétés, RDV par propriétaire). App Claude : le brief s'affiche **dans la conversation** + un
...
> sur les boutons ou valide. **Seul champ texte libre : le nom de l'entreprise.** Si le profil partagé (entreprise, ton,
> CRM) existe déjà (autre skill Eagr installée), **ne le redemande pas**.

1. **Avant la checklist** : vérifie les outils (**Eagr**, **CRM**) et guide la connexion des manquants.
...
2. **La checklist** (pré-cochée) collecte :
   > **Profil** — Entreprise : _(nom)_ · Ton : ☑ Direct & bienveillant · CRM : HubSpot
...
ses membres via **Eagr** (`list_teams_with_members`) ET les propriétaires correspondants côté CRM. Si
...
> **Perf** : lance les membres **en parallèle** ; sur Eagr prends les **résumés/scorecards** (pas les
> transcripts bruts, ~20× plus lourds) ; **plafonne** aux extraits les plus parlants. Capture le **lien
> Eagr** de chaque session citée. Schéma CRM lu à l'install, pas re-déduit ici.

> **Périmètre** : **appels réels (real case)** ET **sessions live coach**. **Exclus le practice** (roleplay).
> **Déduplication** real case ↔ live coach : un même appel est parfois enregistré par les deux ; il ne
> compte qu'**une fois** — garde l'analyse au **score le plus haut**. Clé : `realCaseSessionId` porté par
> la session live coach (fallback : même deal/titre + horodatage et durée quasi identiques).
...
- **Benchmark apples-to-apples** : compare les membres entre eux **à périmètre comparable** (mêmes
  ...
  pertinent, vs d'autres équipes comparables de l'org (`compare_users`). Toujours afficher la base (qui/combien).
- **Transcripts** : 1-2 extraits qui **illustrent l'axe collectif** (idéalement chez 2 personnes
  différentes pour montrer que c'est un pattern d'équipe, pas un cas isolé) — avec le **lien Eagr** cliquable.
...
3. Une fois validés, **mets à jour le journal de coaching d'équipe** : **UNE entrée par équipe** (écrasée,
   ...
   prochain lancement, l'étape 1 évalue ces objectifs et montre la trajectoire.
...
En-tête : nom de l'équipe, effectif analysé, période, et — s'il y a un historique — un mini-rappel
« objectifs d'équipe de la dernière fois ». Puis les blocs activés, en **texte narratif** (prose +
bullets, pas de tableaux de chiffres) :
- 🎯 **Suivi des objectifs d'équipe** — précédents + verdict d'une ligne + pourquoi
- 🩺 **Santé & classement** — le constat en prose (qui tire vers le haut, qui a besoin d'aide ; les chiffres sont dans l'artefact)
- 📈 **Activité CRM** — le constat + les deals d'équipe à traiter en priorité (les compteurs sont dans les cartes)
- 🧭 **Axes collectifs** — les 1-2 compétences communes à travailler (tendance + extraits cités chez ≥ 2 personnes + liens Eagr)
- 📊 **Performance par segment** — en une phrase, où l'équipe décroche
- 🛠️ **Plan de coaching collectif** — les 1-2 priorités de groupe
- 🗓️ **Agenda session collective** — ordre du jour prêt à animer
- 📤 **Résumé équipe** — version partageable
- 🔎 **Focus individuels** — qui mérite un 1:1 dédié (→ `coaching [nom]`)
- ✅ **Objectifs d'équipe proposés** — à valider par le manager (puis mémorisés)
...
l'artefact reste obligatoire). Le texte narratif (avec liens Eagr) reste au-dessus.

Sois factuel, concret, actionnable, et **constructif sur les personnes** (data d'équipe, pas de name-and-shame).
Distingue le **vérifié** de l'**hypothèse**. Si une info n'est pas trouvable, dis-le plutôt que d'inventer.
